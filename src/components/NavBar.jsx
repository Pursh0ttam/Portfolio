     ...
     w-full h-20 px-4 text-white bg-black fixed'>
        ...
        {links.map(({id, link}) =>(
             ...
             className='px-4 cursor-pointer capitalize font-medium text-gray-500
              ...
              >
            <Link to={link} smooth duration={500}>{link}</Link> { /*link hua hai yaha se*/}
            </li>
        ))}
        </ul>
...
<ul className='flex flex-col justify-center items-center 
 absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-500'>
  {links.map(({id,link}) =>(
    ...
    <Link onClick={()=> setNav(!nav)} to={link} smooth duration={500}>{link}</Link> {/**after visiting button icon will be closed */}
